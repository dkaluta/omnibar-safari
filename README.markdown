...
Explicit URLs such as `about:blank`, `file:`, `mailto:`, `tel:`, custom app links, `javascript:`, and `data:` are passed directly to Safari rather than searched. Safari controls which schemes it can open, any permission or app-opening prompts, and any restrictions on internal pages or script URLs. Omnibar does not launch external apps or execute scripts itself. Bare domains, local hostnames with ports, and IP addresses retain normal HTTP(S) inference; search operators such as `site:` remain searches.
